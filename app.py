...
def insert_manual_price(db, asset: str, price: Decimal, source: str="manual"):
    db.add(Price(ts=iso_now_tr(), asset=asset, price=str(price), currency="TRY", source=source, is_stale=0, error_msg=None))
...
def load_transactions_df(db) -> pd.DataFrame:
    ...
    return pd.DataFrame([{
        ...
        "qty": t.qty, "unit_price": t.unit_price, "fee": t.fee,
...
def compute_inventory_and_pnl(tx: pd.DataFrame, price_map: Dict[str, Decimal]) -> pd.DataFrame:
    ...
    for _, r in tx.iterrows():
        ...
        qty = D(r["qty"]); unit_price = D(r["unit_price"]); fee = D(r["fee"])
    ...
    for asset, st_ in state.items():
        ...
        rows.append({"asset":asset,"qty":qty,"avg_cost_try":avg,"last_price_try":cur,"value_try":value,"unrealized_try":unreal,"unrealized_pct":unreal_pct,"realized_try":st_["realized"]})
    for a in ASSETS_META.keys():
        if not any(x["asset"]==a for x in rows):
            rows.append({"asset":a,"qty":Decimal("0"),"avg_cost_try":Decimal("0"),"last_price_try":price_map.get(a),"value_try":Decimal("0"),"unrealized_try":Decimal("0"),"unrealized_pct":None,"realized_try":Decimal("0")})
...
st.markdown(f'<div class="ticker-wrap"><div class="ticker-title">KUYUMCU TABLOSU • Son: {last_ts or "—"} • DB: {get_db_path()}</div><div class="ticker-grid">{card_html}</div></div>', unsafe_allow_html=True)
if stale_assets:
...
with tabs[2]:
    ...
    if tx_df.empty:
        ...
        filt = st.selectbox("Varlık filtresi", ["ALL"]+list(ASSETS_META.keys()), format_func=lambda x: "Hepsi" if x=="ALL" else asset_label(x))
        df = tx_df.copy()
        if filt != "ALL":
            df = df[df["asset"]==filt]
        ...
        st.download_button("CSV Export", data=df.to_csv(index=False).encode("utf-8-sig"), file_name="transactions_export.csv")
...
with tabs[4]:
    ...
    if p.empty:
        ...
        chart = alt.Chart(p).mark_line().encode(
            x="ts:T", y="price_num:Q", color="asset_name:N",
...
with tabs[5]:
    ...
    update_interval = st.number_input("Update interval (dk)", value=int(settings.get("update_interval_min","30")), step=5, min_value=5)
    pnl_thr = st.number_input("PnL alarm eşiği (TRY)", value=float(settings.get("pnl_alert_threshold_try","-5000")), step=500.0)
    fx_primary = st.selectbox("FX Primary", ["exchangerate_host","frankfurter","tcmb"], index=["exchangerate_host","frankfurter","tcmb"].index(settings.get("fx_primary","exchangerate_host")))
    fx_fallback = st.selectbox("FX Fallback", ["frankfurter","exchangerate_host","tcmb"], index=["frankfurter","exchangerate_host","tcmb"].index(settings.get("fx_fallback","frankfurter")))
...
with tabs[7]:
    ...
    v = st.text_input("Fiyat (TRY)", value="0")
    if st.button("Kaydet (manual price)"):
        try:
            vd = D(v)
            if vd <= 0:
